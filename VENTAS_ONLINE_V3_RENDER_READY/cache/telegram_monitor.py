        ...
        if port_num:
            ...
            async def handle_health_check(reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
                try:
                    ...
                    body = json.dumps({
                        "status": "ok",
                        "service": "Telegram Leads Monitor V3",
                        "bot": "online",
                    }).encode("utf-8")
                    resp = (
                        b"HTTP/1.1 200 OK\r\n"
            ...
            try:
                http_server = await asyncio.start_server(handle_health_check, "0.0.0.0", port_num)
                LOGGER.info("Servidor HTTP Render activo en 0.0.0.0:%s", port_num)
                print(f"Servidor HTTP para Render activo en puerto {port_num}")
            except Exception as exc:
                LOGGER.warning("No se pudo iniciar servidor HTTP Render: %s", exc)
...
def self_check() -> dict[str, object]:
    ...
    return {
        "config_ok": bool(SESSION_NAME or SESSION_STRING),
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--run", action="store_true", help="Inicia el monitoreo real de Telegram.")
    parser.add_argument("--check", action="store_true", help="Ejecuta solo el diagnostico local.")
    return parser
...
def main() -> int:
    try:
        ...
        logging.getLogger(__name__).exception("Fallo en ejecucion")
